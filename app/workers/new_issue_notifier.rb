class NewIssueNotifier
  ...
  def self.notify_new_user_location_issue(opts)
    ...
    Notifications.new_user_location_issue(user, issue, category).deliver_now if user.prefs.enable_email
  end
  ...
  def self.notify_new_group_location_issue(opts)
    ...
    issue = Issue.find(opts['issue_id'])
    Notifications.new_group_location_issue(user, group, issue).deliver_now if user.prefs.enable_email
  end
end
