class NewThreadNotifier
  ...
  def self.send_new_group_thread_notification(opts)
    ...
    Notifications.new_group_thread(thread, user).deliver_later if user.prefs.enable_email
  end
  ...
  def self.send_new_user_location_thread_notification(opts)
    ...
    Notifications.new_user_location_issue_thread(thread, user_location).deliver_later if user_location.user.prefs.enable_email
  end
end
