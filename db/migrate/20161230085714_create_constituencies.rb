class CreateConstituencies < ActiveRecord::Migration
  def up
    ...
    update <<~DUMPSQL
      WITH dump AS (
        SELECT id, (ST_DUMP(location)).geom::geometry(Polygon,4326) as geometry FROM constituencies
      )
      UPDATE constituencies SET location = dump.geometry
      FROM dump
      WHERE constituencies.id = dump.id
    DUMPSQL
  end
